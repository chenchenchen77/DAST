...
def apply_chat_template(template, messages, system_message=None, tokenizer:PreTrainedTokenizer=None, add_generation_prompt=False, return_labels=False,Query=None, **tokenization_kwargs):
    """
    ...
    # TODO css add query 2024.7.11
    # ---begin---
    #Query = "Please compress the important information in the several question below\n"
    ...
    # for i, message in enumerate(messages):
    #     """
    #         message["content"]= "Please compress the important information in the several question below\nQuery: " + message['content'].split("\n")[-1] +"\n"+  message['content']
    #     """
    #     if message['role'] == "user":
    #         #if i==0:
    #         #    if "Question:" not in message['content'].split("\n")[-1] +"\n":
    #                 Query = Query + f"Question: {i+1} " + message['content'].split("\n")[-1] +"\n"
    #         #else:
    #         #        Query = Query + message['content'].split("\n")[-1] +"\n"
    #     else:
    ...
    #             Query = Query + message['content'] + "\n"
        ...
        #         else:
        #             Query = Query + message['content'] + "\n"
    #messages[0]['content'] =  Query  + messages[0]['content']
    ...
    #         Query = f"{Query} Question: {content} "
    #         #else:
    #         #    content = message['content'].split("\n")[-1]
    ...
    for i, message in enumerate(messages):
        ...
        # TODO if role is "[INST]" , current is query
        """
    ...
    if tokenizer is not None:
        encoded = tokenizer(conversation, **tokenization_kwargs)
        # TODO css add query length
        # ---begin---
        encoded_query = tokenizer(Query,add_special_tokens=False)['input_ids']
        # Query_length = len(tokenizer(Query, **tokenization_kwargs)["input_ids"])  # TODO 把开始的符号也算上了，如果后面要取出来的话，可以直接[:Query_length]
        # encoded["query_length"] = [len(encoded_query)]
        encoded["query_start"] = [4] #用了llama2的模板，所以这里的start需要设置为4，query需要插入到[INST]之后
        ...
        if return_labels:
            ...
            for i, turn in enumerate(turns):
                ...
                instruction_len = len(tokenizer(user_message, add_special_tokens=False).input_ids) # 得到当前此轮的内容+query
            ...
            # TODO css
            # labels = labels[:1] + [-100] * len(encoded_query) + labels[1:]

            encoded['labels'] = labels.tolist()

            # encoded["query_length"] = None
            # encoded["query_start"] = [0]
            # encoded["query"] = None
            # 对齐label TODO
            encoded['labels'] = encoded['labels'][:4] + [-100] * len(encoded_query) + encoded['labels'][4:]
            # 重新构造conversation
            conversation = f"[INST] {Query}{conversation.split('[INST]')[-1]}"
            # 对齐input和attention_mask
            encoded['input_ids'] = encoded['input_ids'][:4] + encoded_query + encoded['input_ids'][4:]
            encoded['attention_mask'] = encoded['attention_mask'] + [1] * len(encoded_query)
