...
def normalize_text(text, ignore_case=False, ignore_punctuation=True, ignore_space=True, ignore_number=False):
...
class DefaultDataCollator:
    ...
    keys_to_tensorize = {"input_ids", "attention_mask", "labels", "position_ids", "token_type_ids", "length", "depth", "index",'query'}
    ...
    def __call__(self, batch_elem: List) -> Dict[str, Any]:
        ...
        for key, value in first_elem.items():
            ...
            if key in self.keys_to_tensorize:
                if (key=="query" or key=="query_start") and batch_value[0] is None:
                     return_batch[key] = None
                else:
