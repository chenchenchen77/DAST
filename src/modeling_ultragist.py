...
class Memory(torch.nn.Module):
    def __init__(
        ...
        self.rng = np.random.default_rng(42) # self.np.random.default_rng(42).choice(ultragist_ratio, size=1).tolist()
    ...
    def reset(self):
        ...
        self.ultragist_start = 0
        self.ultragist_end = 0

        self.is_third_window = False
        self.index_ultragist_last = []
    ...
    def setQuery(self,query_first,query_length):
    ...
    def finish(self):
        ...
        if not self.kv_finish:
            self.kv_finish = is_finish
        return is_finish
    ...
    def rerank(self,max_compress_tokens,scores_avg):
        dic = [1,2,4,8,16]
        rerank_tokens = []

        for token in self.compress_tokens:
            flag_token = 1
            for i in dic:
                if i > token:
                    break
                flag_token = i
            rerank_tokens.append(flag_token)
        have_token = max_compress_tokens - sum(rerank_tokens)
        ignore_indices = []
        while True:
            filtered_scores = [(i, score) for i, score in enumerate(scores_avg) if i not in ignore_indices]
            if len(filtered_scores) <= 0:
                continue
            try:
                max_index, max_value = max(filtered_scores, key=lambda x: x[1])
            except:
                pass
            if have_token < min(rerank_tokens) or sum(rerank_tokens) == 1024 * len(rerank_tokens):
                break
            if rerank_tokens[max_index] > have_token or rerank_tokens[max_index]==512:
                ignore_indices.append(max_index)
                continue
            have_token -= rerank_tokens[max_index]
            rerank_tokens[max_index] *=2
            ignore_indices.append(max_index)
            if len(ignore_indices) == len(scores_avg):
                ignore_indices  = []
                
        self.compress_tokens = rerank_tokens
  
    def build_compress_tokens(self,choose_ratio=None):
        if choose_ratio is None:
            try:
                alpha = 0.5
                self.ppl = self.ppl[:-1]
                scores_avg = self.total_scores[-1].tolist()
                res = self.ppl  
                max_compress_tokens = self.total_token[-1]
                scores_avg_add_ppl = [(scores_avg[i]* alpha-self.ppl[i] / sum(self.ppl) *alpha)  for i in range(len(scores_avg))]
                scores_avg_add_ppl = nn.functional.softmax(torch.tensor(scores_avg_add_ppl), dim=-1, dtype=torch.float32).tolist()
                compress_tokens_add_ppl = [int((scores_avg_add_ppl[i]) * max_compress_tokens) for i in range(len(scores_avg_add_ppl))]
                
                scores_avg = [(scores_avg[i] * alpha-self.ppl[i] / sum(self.ppl) *alpha)  for i in range(len(scores_avg))]
                scores_avg = nn.functional.softmax(torch.tensor(scores_avg), dim=-1, dtype=torch.float32).tolist()
                self.compress_tokens = [int((scores_avg[i]) * max_compress_tokens) for i in range(len(scores_avg))]
            except:
                pass
            if self.compress_tokens:
                self.rerank(max_compress_tokens,scores_avg)
    def prepare(self, input_ids, attention_mask, labels,query,query_start,total_scores=None):
        """
        ...
        if total_scores:
            self.total_scores = total_scores[0]
            self.ppl = total_scores[1]
            self.build_compress_tokens()
        ...
        if query is not None:
            self.query_length = len(query[0]) 
            self.query_start = query_start[0][0] 
    ...
    def set_compression_ratio(self, start_idx, end_idx):
        ...
        def get_max_length(ratios):
            ...
            (self.max_position_embeddings - self.ultragist_window)  4096-1024
            * condensing_ratio * 压缩率 32 
            + self.ultragist_window) 1024
        ...
        if ratio_mix == "instance-random":
            ...
            if self._ratio is None:
                ...
                if 4096 in max_lengths and self.all_input_ids.shape[1] <= 4096 and False:
                    ultragist_ratio = [0]
                else:
                    future_length = int(ratio_mix.split("-")[1]) # 64
    ...
    def build_total_token(self,cur_size):
        if len(self.total_token) > 0:
            self.total_token.append(cur_size+self.total_token[-1])
        else:
            self.total_token.append(cur_size)
    ...
    def step(self,query):
        ...
        if is_full_window:
            ...
            compression_ratios = self.set_compression_ratio(start_idx=start_idx, end_idx=end_idx) # 得到 一个 压缩倍数


            is_condensing_ratio  = False
            ultragist_sizes = []
            ...
            if self.kv_finish and is_condensing_ratio:
                try:
                    ultragist_sizes = [self.compress_tokens[self.index_compress_tokens]] 
                except:
                    self.compress_tokens
                    self.index_compress_tokens
                self.index_compress_tokens+=1
            self.build_total_token(ultragist_sizes[0]) 
        else:
            self.last_window = True 
        ...
        past_key_values = self.get_memory(
            ...
            ultragist_sizes, #512
            total_ultragist_size, # 512
            raw_size_to_cache, # 0
            window_size # 0
        )
        #(key, value, ultragist_sizes, total_ultragist_size, raw_size_to_cache, window_size)
        """
        (key, value)
        ultragist_sizes [32]
        total_ultragist_size 32
        raw_size_to_cache 0
        window_size 当前窗口大小
        """
        if self.query_first:
            self.ultragist_start += self.query_length
            self.ultragist_end += self.query_length
            self.cur_input  += self.query_length
        ...
        input_ids_ppl = input_ids

        if hasattr(self, "all_labels"):
            labels = self.all_labels[:, self._end_idx: end_idx].to(self._device)
            #labels = torch.cat([labels[:,:1], labels.new_zeros(batch_size, len(query[0])) - 100,labels[:,1:]], dim=1)
        else:
            labels = None
        ...
        # if x == "s":
        #     return
        # if  self.ultragist_end != 0:
        #     if len(self.index_ultragist_last) == 0:
        #         self.index_ultragist_last.append(self.ultragist_end-1)
        #     else:
        #         if self.index_ultragist_last[-1] != self.ultragist_end-1:
        #             self.index_ultragist_last.append(self.ultragist_end-1)
        if not self.is_third_window and len(self.index_ultragist_last) >= 2: #提前添加，所以构造树需要3-1 = 2
            self.is_third_window = True

        if ultragist_sizes[0] > 0 :
            cur_size = ultragist_sizes[0]
        else:
            cur_size = 0
        return input_ids, attention_mask, past_key_values, labels,[self.last_window,self.total_token,self.query_length+self.query_start,cur_size,input_ids_ppl,self.query_first,self.query_start]
    ...
    def update_memory(self, past_key_values):
        ...
        self.ultragist_end += total_ultragist_size
        if self._step_idx == 1:
            self.ultragist_start += 1
            self.ultragist_end  += 1
            self.cur_input +=1
        if len(self.index_ultragist_last) == 0:
            self.index_ultragist_last.append(self.ultragist_end-1)
        else:
            if self.index_ultragist_last[-1] != self.ultragist_end-1:
                self.index_ultragist_last.append(self.ultragist_end-1)
        # print(self.ultragist_start,self.ultragist_end,total_ultragist_size)
    ...
    def _extract_ultragist_and_raw_memory(self, key, value, previous_ultragist_key, previous_ultragist_value, previous_raw_key, previous_raw_value, raw_size_to_cache, total_ultragist_size, ultragist_sizes, ultragist_size_idx):
        ...
        if self.query_first and self.query_length != 0:
            # self.query_first = False
            previous_ultragist_key = cat_tensor([
                previous_ultragist_key,
                slice_tensor(key, start=self.query_start, end=self.query_length+self.query_start, dim=self.k_seq_dim).to(target_device, non_blocking=True)
            ], dim=self.k_seq_dim)
            previous_ultragist_value = cat_tensor([
                previous_ultragist_value,
                slice_tensor(value, start=self.query_start, end=self.query_length+self.query_start, dim=self.v_seq_dim).to(target_device, non_blocking=True)
            ], dim=self.v_seq_dim)
