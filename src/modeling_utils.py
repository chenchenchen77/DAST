...
@torch.no_grad()
def evaluate_perplexity(model, dataloader, tokenizer,accelerator:Optional[Accelerator]=None):
    ...
    for i, x in enumerate(tqdm(dataloader, desc="Computing Perplexity")):
        ...
        #                               input_shape=(1, x["input_ids"].shape[1]),
        #                               transformer_tokenizer=tokenizer)
        # flops, _ = thop.profile(model, inputs=(x["input_ids"].shape))
        # print(flops)
        # exit(0)
        output = model(**x)
