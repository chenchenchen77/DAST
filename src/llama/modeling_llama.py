...
class LlamaAttention(nn.Module):
    ...
    def forward(
        ...
        pre_forward:Optional[bool] = None,
        **kwargs,
...
class LlamaSdpaAttention(LlamaAttention):
    ...
    def forward(
        ...
        info: Optional[list] = None,
        pre_forward:Optional[bool] = None,
        is_last_decoder:Optional[bool] = None,
    ) -> Tuple[torch.Tensor, Optional[torch.Tensor], Optional[Tuple[torch.Tensor]]]:
        ...
        if window_size > 0:
            past_key_value = (key_states, value_states, ultragist_sizes, total_ultragist_size, raw_size_to_cache, window_size)
        if past_key is not None:
            caf_key_states = past_key.clone()#.detach()
            caf_value_states = past_value.clone()#.detach()
            key_states = torch.cat([past_key, key_states], dim=2)
            value_states = torch.cat([past_value, value_states], dim=2)
        # NOTE: window_size == 0 indicates the ultragist is disabled, the model works as is, so the new past_key_values should concatenate old ones
        if window_size == 0:
        ...
        if pre_forward and info[0] and is_last_decoder:
            a =  1
            if info[2] != -1:
                total_token = [info[2]]
            else:
                total_token = [0]
            compress_token = info[1]
            for i in range(len(compress_token)):
                total_token.append(total_token[0]+compress_token[i])
            weights = torch.matmul(query_states,key_states.transpose(-1,-2))[:,:,-1,:] / math.sqrt(self.head_dim)
            weights = weights.mean(dim=1)[0]#.tolist()
            res = []

            for i in range(len(total_token) - 1):
                start = total_token[i]
                end = total_token[i + 1]
            scores = torch.tensor(res)
        ...
        attn_output = attn_output.view(bsz, q_len, self.num_heads, self.head_dim).transpose(1, 2)
        ...
        return attn_output, None, past_key_value,scores
...
class LlamaDecoderLayer(nn.Module):
    ...
    def forward(
        ...
        info: Optional[list] = None,
        pre_forward: Optional[bool] = False,
        is_last_decoder: Optional[bool] = False,
        **kwargs,
        ...
        # Self Attention
        hidden_states, self_attn_weights, present_key_value,scores = self.self_attn(
            hidden_states=hidden_states,
            attention_mask=attention_mask,
            position_ids=position_ids,
            past_key_value=past_key_value,
...
class LlamaModel(LlamaPreTrainedModel):
    ...
    def forward(
        ...
        if past_key is not None:
            ...
            #past_key_values_length = past_key_values_length + query.shape[1]
        ...
        if total_ultragist_size > 0:
            ...
            #query_inputs_embeds = self.embed_tokens(query)
        ...
        for idx, decoder_layer in enumerate(self.layers):
            ...
            if self.gradient_checkpointing and self.training:
                layer_outputs,scores = self._gradient_checkpointing_func(
                    ...
                    output_attentions,
                    use_cache,
                    info = info,# if idx+1 == len(self.layers) else None,
                    pre_forward = pre_forward,
                    is_last_decoder=is_last_decoder
                )
            else:
                layer_outputs,scores = decoder_layer(
                    hidden_states,
                    attention_mask=attention_mask,
                    position_ids=position_ids,
                    past_key_value=past_key_value,
...
class LlamaForCausalLM(LlamaPreTrainedModel):
    ...
    def from_pretrained(cls, *args, **kwargs):
        ...
        query_first = True
        query_length = 0
        model.memory.setQuery(query_first,query_length)
    ...
    def _native_forward(
        ...
        outputs,total_scores = self.model(
            input_ids=input_ids,
            attention_mask=attention_mask,
            position_ids=position_ids,
        ...
        ppl = True
        perplexity = None
        cur_size,input_ids_ppl,query_first,query_end = info[-4],info[-3],info[-2],info[2]
        if ppl and not info[0] and pre_forward and False:
            if cur_size > 0:
                pred = logits[:,-cur_size:,:][:,:-1,:] # 预测输出
                input_ids_ppl = input_ids_ppl[:,-cur_size:]
                labels_ = input_ids_ppl[:,1:]  # 随机生成标签
                # print(labels_.shape)
                loss_ppl = compute_loss(pred,labels_)[0]
                # 计算困惑度
                perplexity = torch.exp(loss_ppl)
                print(perplexity)
        if ppl and not info[0] and pre_forward:
            # if info[1]
            if cur_size > 0:
                pred = logits[:,:-cur_size,:][:,:-1,:] # 预测输出
            else:
            ...
            if query_first:
                ...
                # print(pred.shape)
                pred = pred[:,query_end:,:]
                input_ids_ppl = input_ids_ppl[:,query_end:]
            labels_ = input_ids_ppl[:,1:]  # 随机生成标签
            # print(labels_.shape)
            loss_ppl = compute_loss(pred,labels_)[0]
            # 计算困惑度
    ...
    def _pre_forward(self,
        ...
        output_hidden_states: Optional[bool] = None,
        return_dict: Optional[bool] = None,
        pre_forward: Optional[bool] = True,
        query: torch.LongTensor = None,
        query_start: list = None,
    ):
        self.memory.prepare(
            input_ids=input_ids,
            attention_mask=attention_mask,
        ...
        while not self.memory.finish:
            input_ids, attention_mask, past_key_values, labels,info = self.memory.step(query)
            outputs,total_scores,ppl = self._native_forward(
                input_ids=input_ids,
                attention_mask=attention_mask,
                position_ids=position_ids,
                past_key_values=past_key_values,
            ...
            if ppl:
                total_ppl.append(ppl.tolist())
            else:
                total_ppl.append(None)
            self.memory.update_memory(outputs.past_key_values)

        return outputs,total_scores,total_ppl
    ...
    def _ultragist_forward(self,
        ...
        query: torch.LongTensor = None,
        query_start: list = None,
        total_scores: list = None,
    ):
        # t1 = time.time()
        # initialize cache
        # first_query = True
        # 如果是True的话，后续的step部分取到的 past_key_values 中的query跟应该为0
        a=1
        self.memory.prepare(
            input_ids=input_ids,
            attention_mask=attention_mask,
        ...
        while not self.memory.finish:
            ...
            outputs,_,_ = self._native_forward(
                input_ids=input_ids,
                attention_mask=attention_mask,
                position_ids=position_ids,
                past_key_values=past_key_values,
    ...
    def forward(self, **kwargs):
        ...
        with optional_grad_ctx(with_grad=self.training):
            ...
            if hasattr(self, "_enable_ultragist") and self._enable_ultragist == False:
                ...
                if not self.memory.kv_finish:#只对input处理
                    _,attn,ppl = self._pre_forward(**kwargs)#⬇预先forward一次，返回一个ultragist_size
                    self.memory.pre_reset() #重新reset，进行training or inference
                    kwargs['total_scores'] = [attn,ppl]
                    # print("1")
                # print()
                return self._ultragist_forward(**kwargs)

    def prepare_inputs_for_generation(
        self, input_ids, query=None,query_length=None,query_start=None,past_key_values=None, attention_mask=None, inputs_embeds=None, **kwargs
    ):
